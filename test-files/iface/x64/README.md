...
2.  `Consumer.hi`, `LogHelper.hi` and `Main.hi` used in the tests of
    Backpack-related `*.hi` files.
...
See the Stack project located at `test-files\iface\x64\backpack` and documented
in file `README.md` in that project directory.

### On Windows (Powershell)

1.  Move to that Stack project's project directory.

2.  Build the project but override the specified `snapshot` to specify the
...
3.  From the project directory, copy the three generated `*.hi` to the desired
